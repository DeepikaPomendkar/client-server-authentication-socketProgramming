...
class Client:
    ...
    def take_action(self, action_name):
        ...
        if action_name == actions.QUIT_ACTION or len(action_name) == 0:
            ...
            if self.nonce_send:
                hashed_password = passwords.find_hash_password(input_line, self.salt)[0]      # hash pass
                input_line = passwords.find_hash_password(hashed_password, self.nonce)[0]     # add nonce and hash again
                self.nonce_send = False
